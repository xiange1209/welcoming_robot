...
namespace lslidar_driver {
    ...
    bool LslidarCxDriver::initAngleConfig() {
        ...
        // 计算可用区间
        std::vector<LslidarCxDriver::AngleRange> usableAngles{};
        int lastEnd = 0;
    ...
    bool LslidarCxDriver::initialize() {
        ...
        this->initTimeStamp();
        LS_MSG << "init_config_finish" << LS_END;
    ...
    // 合并重叠的角度区间
    std::vector<LslidarCxDriver::AngleRange> LslidarCxDriver::mergeRanges(std::vector<LslidarCxDriver::AngleRange>& ranges) {
        if (ranges.empty()) return {};
        std::vector<LslidarCxDriver::AngleRange> mergedRanges;
    ...
    // 计算非禁用的角度区间
    std::vector<LslidarCxDriver::AngleRange> LslidarCxDriver::calculateDisableAngles(const std::vector<long int>& disableMin, const std::vector<long int>& disableMax) {
        std::vector<LslidarCxDriver::AngleRange> allRanges;
