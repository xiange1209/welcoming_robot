...
class MapServiceCcNode(Node):
    ...
    def __init__(self):
        ...
        #     - 要比抑制逾時（frontier_suppression_timeout_s 300 秒）短：全部都被抑制時，
        ...
        # ------------------------------------------------------------------
        self.lc_map_server = LifecycleNodeClient(self, "map_server", self.client_cb_group, transition_timeout)
        self.lc_map_saver = LifecycleNodeClient(self, "map_saver", self.client_cb_group, transition_timeout)
        self.lc_amcl = LifecycleNodeClient(self, "amcl", self.client_cb_group, transition_timeout)
        self.lc_slam = LifecycleNodeClient(self, "slam_toolbox", self.client_cb_group, transition_timeout)
        ...
        self.create_service(Trigger, "get_nav_mode", self._get_nav_mode_callback, callback_group=self.client_cb_group)
        # 導航前重新對齊位姿。IMU 零偏會讓車子靜置時 yaw 一直漂 (見 _amcl_refresh_tick)，
        # 導航起步前對一次，才不會照著錯的角度規劃。
        self.create_service(Trigger, "align_pose", self._align_pose_callback, callback_group=self.client_cb_group)
        ...
        if self.amcl_refresh_period_sec > 0:
            self.create_timer(
                self.amcl_refresh_period_sec,
                self._amcl_refresh_tick,
                callback_group=self.client_cb_group,
            )
            ...
            if pose_known:
                ...
                if fallback is None:
                    ...
                    self.get_logger().warn(
                        f"沒有已知位姿，改用地圖中心 ({x:.2f}, {y:.2f})，" "請執行全域定位讓 AMCL 收斂"
                    )
    ...
    def _create_map_goal_callback(self, goal_request):
        ...
        if self._exploration_active:
            self.get_logger().warning(
                f"建立地圖被拒絕: 已經有建圖作業在進行中" f"（要重新開始請先按「結束並儲存」或取消目前的作業）"
            )
            return GoalResponse.REJECT
    ...
    def _wait_for_mapping_done(self, goal_handle, deadline=None) -> str:
        """等建圖結束，回傳 'done' / 'cancel' / 'timeout' / 'stalled'
    ...
    def _align_pose_with_scan(self, pose):
        ...
        def score(bx, by, byaw):
            ...
            for dx, dy in pts:
                ...
                if (
                    (gx, gy) in occupied
                    or (gx + 1, gy) in occupied
                    or (gx - 1, gy) in occupied
                    or (gx, gy + 1) in occupied
                    or (gx, gy - 1) in occupied
                ):
                    hit += 1
            return hit / len(pts)
        ...
        self.get_logger().info(
            f"位姿對齊：吻合度 {base_score * 100:.0f}% -> {best[0] * 100:.0f}%，" f"角度修正 {delta:+.1f}°"
        )
    ...
    def _amcl_refresh_tick(self) -> None:
        ...
        if not self.nomotion_update_client.service_is_ready():
            return
        try:
            wait_for_future(self.nomotion_update_client.call_async(EmptySrv.Request()), timeout_sec=2.0)
        except Exception:  # noqa: BLE001
            pass  # 偶爾逾時無所謂，下一輪會再試
    ...
    def _align_pose_callback(self, _request, response):
        ...
        response.success = True
        response.message = f"已對齊到 ({aligned[0]:.2f}, {aligned[1]:.2f}, " f"{math.degrees(aligned[2]):.1f}°)"
        return response
    ...
    def _switch_map_callback(self, request, response):
        try:
            ...
            if self._exploration_active or self.mode == MODE_MAPPING:
                ...
                self.get_logger().warn(f"拒絕在建圖進行中切換地圖 (要求切到 {request.map_id})")
                return response
