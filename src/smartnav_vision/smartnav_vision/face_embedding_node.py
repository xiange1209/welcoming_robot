...
class FaceEmbeddingNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter(
            ...
            640,
            ParameterDescriptor(
                description="偵測輸入邊長。不給的話 InsightFace 會跑 " "[(128,128),(640,640)] 串聯，等於每幀偵測兩次"
            ),
        )
        ...
        self.declare_parameter(
            ...
            2,
            ParameterDescriptor(
                description="人臉推論最多用幾顆核心（0 = 不限）。" "Pi 4 只有 4 顆，不限的話會搶走導航控制迴圈"
            ),
        )
        ...
        self._interval_idle = float(self.get_parameter("process_interval_sec").get_parameter_value().double_value)
        self._interval_nav = float(
            self.get_parameter("process_interval_navigating_sec").get_parameter_value().double_value
        )
        self._skip_when_nav = bool(self.get_parameter("skip_while_navigating").get_parameter_value().bool_value)
        ...
        def _on_param(params):
            for p in params:
                if p.name == "process_interval_sec":
                    ...
                    self.get_logger().info(f"導航中{'完全跳過' if self._skip_when_nav else '改用降頻'}")
    ...
    def _nav_active_cb(self, msg: Bool) -> None:
        if msg.data != self._navigating:
            ...
            if msg.data and self._skip_when_nav:
                ...
                self.get_logger().info("導航開始 -> **退訂影像、完全停止人臉推論**" "（相機仍開著，平板照樣有畫面）")
            elif msg.data:
                self.get_logger().info(f"導航開始 -> 人臉處理間隔 {self._interval_nav:.1f} 秒")
            else:
                self._set_image_sub(True)
                ...
                self.get_logger().info(
                    f"導航結束 -> 重新訂閱影像、恢復辨識，間隔 {iv:.1f} 秒" f"{'（不限制）' if iv <= 0 else ''}"
                )
    ...
    def _nav_watchdog(self) -> None:
        ...
        if now - self._last_report >= 30.0:
            ...
            if self._skip_when_nav:
                self.get_logger().info("（導航中：已退訂影像、暫停人臉推論，節點正常）")
            else:
                self.get_logger().info(
                    f"（導航中：降頻 {self._interval_nav:.1f} 秒一幀，" f"已處理 {self._n_done}／跳過 {self._n_skip}）"
                )
    ...
    def _image_capture_callback(self, msg: CompressedImage) -> None:
        ...
        if now - self._last_report >= 30.0:
            ...
            self.get_logger().info(
                f"人臉處理 {self._n_done}/{tot} 幀" f"（跳過 {self._n_skip}），目前間隔 {interval:.1f} 秒"
            )
            self._n_done = self._n_skip = 0
            self._last_report = now
        try:
