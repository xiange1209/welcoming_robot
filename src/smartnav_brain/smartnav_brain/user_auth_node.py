...
class UserAuthNode(Node):
    ...
    def __init__(self):
        ...
        #
        # ★★ 2026-09-18：2.0 -> 5.0。**原本的推導是用一個已經不存在的架構算的** ★★
        #
        # 原註解：「相機 15+ FPS，正常情況每 66 ms 就有一幀；2 秒等於連續 30 幀都沒臉」。
        # 那個 15 FPS 是 2026-07-29 **已刪除**的舊 vision 架構量的
        # （CLAUDE.md 第 5 條 9/06 已作廢它，但這裡的推導沒有被連帶更新）。
        #
        # 現行 `face_embedding_node` 的實際節奏：
        #   單幀推論約 1004 ms（1.92 核心秒，`車子/人臉模型基準_20260819.md`）
        #   待機 process_interval_sec 預設 1.0 -> 實測偵測產出 **0.97 Hz**
        #
        # 所以「2 秒」真正的意思是 **2 幀**，不是 30 幀 —— 安全邊際被高估了 15 倍。
        # 而 `face_embedding_node` **只在偵測到臉時才發布**，所以只要連續漏偵測兩次，
        # `_identity_timeout_tick` 就把身份清成 Unknown / recognized=False。
        # 現場症狀：VIP 迎賓講到一半身份掉回訪客、平板信心值歸零，而且反覆發生。
        #
        # 5.0 秒 = 待機時約 5 幀，把原本「不要被偶發漏失誤觸發」的意圖
        # 按真實吞吐量重新換算。代價是「人走掉」的偵測慢 3 秒 ——
        # 在 1 Hz 的吞吐量下這兩件事無法兼得，而迎賓場景誤清身份的代價明顯比較大。
        #
        # ⚠ 導航中另當別論：`skip_while_navigating` 預設 true，
        #   導航期間本來就完全不推論，身份必然逾時。那是刻意的（帶位時不認人）。
        self.declare_parameter(
            "identity_timeout_sec",
            5.0,
            ParameterDescriptor(
                description="超過這麼久沒收到人臉向量就發布『無人』身份；0 = 關閉。★ 要配合 face_embedding 的 process_interval_sec，別讓它小於幾個幀間隔"
            ),
        )
        ...
        self.identity_timeout = float(self.get_parameter("identity_timeout_sec").get_parameter_value().double_value)
    ...
    def _identity_timeout_tick(self) -> None:
        ...
        self.get_logger().info(f"👤 已連續 {self.identity_timeout:.1f} 秒沒偵測到人臉，身份清除為『無人』")

    def _publish_identity(self, user_uuid, user_info, face_msg: FaceEmbedding, similarity: float = 0.0) -> None:
        """發布身份辨識結果"""
    ...
    def _get_prototypes(self) -> dict:
        ...
        """
        key = tuple(
            sorted((uuid, len(info.get("samples", []))) for uuid, info in self.user_manager.user_registry.items())
        )
        ...
        for user_uuid, embeddings in self.user_manager.get_all_embeddings().items():
            ...
            protos[user_uuid] = cast(np.ndarray, np.mean(embeddings / norms, axis=0, dtype=np.float32))
