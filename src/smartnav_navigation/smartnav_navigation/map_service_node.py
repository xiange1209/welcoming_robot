...
class MapServiceNode(Node):
    ...
    def __init__(self):
        ...
        # 建圖的最長等待時間。這段時間內要涵蓋 nav2 生命週期節點全部 active
        # (global_costmap 得等 slam_toolbox 起來才拿得到 map -> base_footprint 的 TF，
        # 在 Pi 4 上就要一兩分鐘) 再加上實際跑完整個空間的時間，原本寫死的 400 秒不夠用。
        self.declare_parameter("exploration_timeout_sec", 1200.0)
        self.exploration_timeout_sec = self.get_parameter("exploration_timeout_sec").value
        ...
        # Nav2 生命週期管理客戶端
        self.map_server_get_state_client = self.create_client(
            GetState, "/map_server/get_state", callback_group=self.client_cb_group
        )
        ...
        self.map_server_change_state_client = self.create_client(
            ChangeState, "/map_server/change_state", callback_group=self.client_cb_group
        )
    ...
    def _init_callback(self) -> None:
        ...
        # map_server 的生命週期改由本節點掌管，要先啟用它才會出現 load_map 服務
        self._wait_for_service("map_server_get_state", self.map_server_get_state_client)
        self._wait_for_service("map_server_change_state", self.map_server_change_state_client)
        self.ensure_map_server_active()
        self._wait_for_services()
        self.nav2_load_map(None)

    def _wait_for_service(self, service_name: str, client) -> None:
        """等待單一服務就緒"""
    ...
    def _wait_for_services(self) -> None:
        ...
        services = [
            ...
            ("map_server_get_state", self.map_server_get_state_client),
            ("amcl_get_state", self.amcl_get_state_client),
            ("slam_get_state", self.slam_get_state_client),
            ("map_server_change_state", self.map_server_change_state_client),
        ...
        for service_name, client in services:
            self._wait_for_service(service_name, client)
    ...
    def create_map_callback(self, goal_handle):
        ...
        try:
            ...
            # 清掉上一輪殘留的完成旗標，否則會立刻跳出等待迴圈並存到舊地圖
            self.exploration_finished = False
            ...
            max_wait_time = Duration(seconds=self.exploration_timeout_sec)
            while True:
            ...
            try:
                # map_server 已退場，map_saver 得等 slam_toolbox 發出第一張地圖，
                # 這裡要留得比 map_saver 的 save_map_timeout 更寬
                self._wait_for_future(future, timeout_sec=20.0)
            except Exception as e:
                result.map_info = MapInfo()
                result.success = False
            ...
            # 地圖已存檔，把 map_server 拉回 active 才能重新載入地圖
            self.ensure_map_server_active()
            ...
            # 失敗或取消時 map_server 可能仍停在 unconfigured，
            # 此時 /map_server/load_map 並不存在，後續切換地圖都會失敗
            self.ensure_map_server_active()
    ...
    def _change_node_state(self, client, transition_id: int, timeout_sec: float = 10.0) -> None:
        """對生命週期節點送出狀態轉換請求"""
    ...
    def switch_to_slam_mode(self) -> bool:
        ...
        map_server 必須一路退到 unconfigured。只做 deactivate 並不會銷毀它的
        publisher，舊地圖仍會以 transient local 的形式留在 /map 上，讓 map_saver
        訂閱時搶先收到上一張地圖而存錯檔。
        """
        map_server_state = self.get_node_state(self.map_server_get_state_client)
        amcl_state = self.get_node_state(self.amcl_get_state_client)
        slam_state = self.get_node_state(self.slam_get_state_client)

        if map_server_state == "unconfigured" and amcl_state == "inactive" and slam_state == "active":
            return True

        try:
            if map_server_state == "active":
                self._change_node_state(self.map_server_change_state_client, Transition.TRANSITION_DEACTIVATE)
                map_server_state = "inactive"
            # 這裡必須是 if 而不是 elif：active 的 map_server 只做 deactivate 會停在 inactive，
            # publisher 沒被銷毀，舊地圖仍以 transient local 留在 /map 上，
            # map_saver 一訂閱就會先收到那張舊地圖而存錯檔 (正是上面 docstring 說的狀況)。
            if map_server_state == "inactive":
                self._change_node_state(self.map_server_change_state_client, Transition.TRANSITION_CLEANUP)

            if amcl_state == "active":
                self._change_node_state(self.amcl_change_state_client, Transition.TRANSITION_DEACTIVATE)

            if slam_state != "active":
                if slam_state == "unconfigured":
                    self._change_node_state(self.slam_change_state_client, Transition.TRANSITION_CONFIGURE)

                self._change_node_state(self.slam_change_state_client, Transition.TRANSITION_ACTIVATE)
    ...
    def ensure_map_server_active(self) -> bool:
        """確保 map_server 回到 active

        建圖期間 map_server 被退到 unconfigured，此時 /map_server/load_map 服務並不存在，
        因此載入或發布地圖前都必須先把它拉回 active。
        """
        state = self.get_node_state(self.map_server_get_state_client)
        if state == "active":
            return True

        try:
            if state == "unconfigured":
                self._change_node_state(self.map_server_change_state_client, Transition.TRANSITION_CONFIGURE)
                state = self.get_node_state(self.map_server_get_state_client)

            if state != "inactive":
                self.get_logger().error(f"map_server 處於非預期狀態 {state}，無法啟用")
                return False

            self._change_node_state(self.map_server_change_state_client, Transition.TRANSITION_ACTIVATE)
            self.get_logger().info("成功啟用 map_server")
        except Exception as e:
            self.get_logger().error(f"啟用 map_server 失敗: {str(e)}")
            return False
    ...
    def rosout_callback(self, msg: Log) -> None:
        ...
        if msg.name == "frontier_explorer":
            if "Exploration finished" in msg.msg:
