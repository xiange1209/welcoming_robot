...
function ScenarioCard({ active, onApplied }: { active: boolean; onApplied: () => void }) {
  ...
  useEffect(() => {
    ...
    loaded.current = true;
    void api<ApiResult & { scenarios?: Scenario[] }>("/api/scenarios").then((r) =>
      setList(r.scenarios || []),
    );
  }, [active]);
  ...
  const apply = async (s: Scenario) => {
    ...
    const r = await post<ApiResult & { steps?: string[] }>(`/api/scenarios/${s.key}`);
  ...
  return (
    <Card title="測試情境" className="flex-none">
      <Hint className="mb-2.5">
        ...
        <b className="text-label-2">點名稱可以先看它會做什麼</b>——有幾條「要關掉」的理由不直覺。
      </Hint>
        ...
        {list.map((s) => (
          ...
          >
            <div className="flex items-center gap-2.5 px-3 py-2.5">
              ...
              <Button variant="primary" disabled={!!busy} onClick={() => void apply(s)}>
                {busy === s.key ? "套用中…" : "套用"}
              </Button>
            </div>
            ...
            {open === s.key && (
              <div className="border-t border-[var(--color-outline-variant)] px-3 py-2 text-[12px] leading-relaxed">
                {s.stop.length > 0 && (
                  <div>
                    <span className="mr-2 font-semibold text-red">先停止</span>{s.stop.join("、")}
                  </div>
                )}
                <div>
                  <span className="mr-2 font-semibold text-green">再啟動</span>{s.start.join("、")}
                </div>
      ...
      {steps.length > 0 && (
        <div className="mt-2.5 text-[12px] leading-relaxed text-label-3">
          {steps.map((line, i) => (
            <div
              key={i}
              className={cx(line.startsWith("✗") && "text-red", line.startsWith("✓") && "text-green")}
            >
              {line}
            </div>
          ))}
...
function VerifyCard({ active }: { active: boolean }) {
  ...
  const run = async (v: VerifyItem) => {
    ...
    const r = await post<ApiResult & { output?: string }>(`/api/verify/${v.key}`);
  ...
  return (
    <Card title="一鍵驗證" className="flex-none">
      <Hint className="mb-2.5">
        按下去會實際<b className="text-label-2">量</b>，不是讀設定值。從 V0 依序往下，
          ...
          <Row key={v.key}>
            <div className="min-w-0 flex-1">
              <div className="font-semibold">
                {v.label}
                {!v.runnable && (
                  <span className="ml-1.5 text-[12px] font-normal text-label-3">（要鍵盤輸入）</span>
                )}
              </div>
              <div className="text-[12px] text-label-3">{v.why}</div>
              {v.note && <div className="text-[12px] text-label-3">{v.note}</div>}
            </div>
            <Button
