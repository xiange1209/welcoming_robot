...
class SpeechRecognizerNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter(
            "min_text_length", 2, ParameterDescriptor(description="短於這個字數的最終結果直接丟棄（雜訊）")
        )
        self.declare_parameter(
            "drop_uniform_text", True, ParameterDescriptor(description="整句都是同一個字時丟棄，例如『嗯嗯嗯嗯』")
        )
        self.declare_parameter(
            "collapse_repeats",
            3,
        ...
        self.declare_parameter(
            "enable_corrections", True, ParameterDescriptor(description="是否套用內建的銀行詞彙同音修正表")
        )
        ...
        self.declare_parameter(
            "queue_warn_depth", 20, ParameterDescriptor(description="音訊佇列積壓超過這個數就警告（上限 50）")
        )
        ...
        self.declare_parameter(
            "echo_overlap_ratio", 0.8, ParameterDescriptor(description="重疊比例門檻。太低會把客人複述的話也丟掉")
        )
        ...
        self.declare_parameter(
            ...
            6,
            ParameterDescriptor(
                description="與機器人剛講的話共同前綴達這麼多字就算回音（抓 ASR 聽爛後半段的情況）；0 = 關閉"
            ),
        )
        ...
        self.declare_parameter(
            "rule1_min_trailing_silence", 2.4, ParameterDescriptor(description="還沒解出任何字時，靜音多久算句尾（秒）")
        )
        self.declare_parameter(
        ...
        self.declare_parameter(
            "hotwords_score", 1.8, ParameterDescriptor(description="熱詞加分權重。太高會讓模型把不相干的話也扭曲成熱詞")
        )
        self.declare_parameter(
            "max_active_paths",
            4,
        ...
        self.mute_during_tts = self.get_parameter("mute_during_tts").get_parameter_value().bool_value
        self.echo_filter_sec = self.get_parameter("echo_filter_sec").get_parameter_value().double_value
        self.echo_overlap = self.get_parameter("echo_overlap_ratio").get_parameter_value().double_value
        self.echo_prefix_chars = int(self.get_parameter("echo_prefix_chars").get_parameter_value().integer_value)
        self._robot_said = []  # [(時刻, 正規化後的字串)]
        self._echo_drops = 0
        self.prefer_int8_model = self.get_parameter("prefer_int8_model").get_parameter_value().bool_value
        self.use_sherpa_endpoint = self.get_parameter("use_sherpa_endpoint").get_parameter_value().bool_value
        self.rule1_min_trailing_silence = (
            self.get_parameter("rule1_min_trailing_silence").get_parameter_value().double_value
        )
        self.rule2_min_trailing_silence = (
            self.get_parameter("rule2_min_trailing_silence").get_parameter_value().double_value
        )
        self.rule3_min_utterance_length = (
            self.get_parameter("rule3_min_utterance_length").get_parameter_value().double_value
        )
        ...
        self._tts_expire_timer = self.create_timer(0.2, self._tts_expire, callback_group=self.callback_group)
    ...
    def _init_sherpa_onnx_asr(self) -> None:
        ...
        try:
            ...
            if asr_model_dir:
                ...
                for label, f in (("encoder", encoder_file), ("decoder", decoder_file), ("joiner", joiner_file)):
                    prec = "int8" if ".int8." in f.name else "fp32"
                    self.get_logger().info(f"    {label}: {prec}  {f.stat().st_size / 1024 / 1024:.0f} MB  ({f.name})")
                ...
                kwargs = dict(
                    ...
                    max_active_paths=self.get_parameter("max_active_paths").get_parameter_value().integer_value,
                ...
                if hot_file:
                    try:
                        ...
                        if clean:
                            ...
                            #   空的話那條規則會完全不作用**而且不報錯**。
                            self.get_logger().info(f"  後處理用的熱詞集合：{len(_hw)} 個" f"（範例 {sorted(_hw)[:4]}）")
                            self.get_logger().info(f"熱詞檔已濾除註解：{len(clean)} 個詞（原檔 {hot_file}）")
                            hot_file = tf.name
                        else:
                ...
                if hot_file:
                    try:
                        ...
                        self.get_logger().info(f"✓ 熱詞已啟用：{hot_file}（score {hot_score:.1f}）")
                    except Exception as e:  # noqa: BLE001
                        # 不同 sherpa-onnx 版本的參數名不一定相同。熱詞是加分項，
                        # 不能因為它讓整個 ASR 起不來 —— 退回沒有熱詞的版本。
                        self.get_logger().warning(f"熱詞載入失敗（沿用無熱詞模式，辨識仍可用）: {e}")
                        recognizer = None
        ...
        try:
            ...
            if not is_final and self._sherpa_endpoint_ok:
                try:
                    ...
                    self.get_logger().warning(f"sherpa 端點偵測不可用，改由 VAD 單獨斷句（辨識不受影響）: {exc}")
                    ...
                    if is_final or endpoint:
                        ...
                        if cleaned and self._is_echo(cleaned):
                            ...
                            self.get_logger().info(
                                f"🔁 丟棄回音（機器人自己講的）: '{cleaned}'" f"（累計 {self._echo_drops} 次）"
                            )
                            cleaned = None
                        if cleaned:
    ...
    def _is_echo(self, text: str) -> bool:
        ...
        for said_t, said in self._robot_said:
            ...
            if ratio >= self.echo_overlap:
                self.get_logger().info(
                    f"⊘ 判定為回音（bigram 重疊 {ratio:.2f} >= {self.echo_overlap:.2f}）：" f"'{text}' ~ '{said}'"
                )
                return True
    ...
    def _tts_cb(self, msg: String) -> None:
        ...
        if self.echo_filter_sec > 0 and msg.data:
            ...
            self._robot_said = [(t, x) for (t, x) in self._robot_said if now - t <= self.echo_filter_sec]
        if not self.mute_during_tts:
            return  # 走文字層過濾，不丟音訊（客人隨時能插話）
        secs = min(len(msg.data) * self.TTS_SEC_PER_CHAR + self.TTS_TAIL_SEC, self.TTS_MAX_MUTE_SEC)
    ...
    def _postprocess(self, text: str) -> Optional[str]:
        ...
        if len(out) >= 4:
            ...
            for _n in (4, 3, 2):  # 先長後短，避免長詞被短詞拆散
                ...
                _buf = []
                while _i < len(out):
                    if _i + 2 * _n <= len(out) and out[_i : _i + _n] == out[_i + _n : _i + 2 * _n]:
        ...
        **立刻解除靜音** —— 真訊號等於沒作用。所以 True 時要一併設定天花板。
        ...
        with self._playing_lock:
            ...
            if self.is_playing:
                ...
                self._playing_until = 0.0
        self.get_logger().info("🔇 平板回報開始朗讀，辨識靜音" if msg.data else "🔊 平板回報朗讀結束，恢復辨識")
