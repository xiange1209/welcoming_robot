...
class VoiceTriggerNode(Node):
    ...
    def _build_mic_mixer(self):
        ...
        if mode == "cancel":
            is_default = len(coeffs) == len(DEFAULT_COEFFS) and all(
                abs(a - b) < 1e-9 for a, b in zip(coeffs, DEFAULT_COEFFS)
            )
            src = "內建驗證過的預設值" if is_default else "外部覆寫"
            self.get_logger().info(
                f"    FIR {len(coeffs)} 抽頭（{src}）  pre={pre}  " f"係數最大絕對值 {max(abs(c) for c in coeffs):.2f}"
            )
            self.get_logger().info("    預期：底噪 -13.6 dB、人聲 -5.4 dB -> SNR 淨賺 +8.2 dB（正前方）")
        elif mode == "beamform":
    ...
    def _playback_cb(self, msg: Bool) -> None:
        ...
        if in_command:
            ...
            self.get_logger().info(f"🔀 TTS {'開始' if active else '結束'}，強制切句（音訊照收，客人仍可插話）")
