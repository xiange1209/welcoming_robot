...
專題/
├── src/
│   ├── ai_vision/           # Face detection, recognition, liveness (使用者)
...
python3 src/scripts/manage_vip_database.py add-vip \
  --name "使用者" --gender M --level platinum \
...
2. **Codex reads code and designs solution**
   - Analyzes AGENTS.md and architecture
   - Reads relevant files
   - Generates/modifies code in Windows directory

3. **You transfer updated code to Raspberry Pi**
   ```bash
   cd C:\Users\使用者\Desktop\專題
...
5. **You report results back to Codex**
...
- **AGENTS.md** (this file): Architecture for Codex
...
| **docs/vibe_coding流程.md** | Complete development workflow with Codex (read this for dev process!) |
