...
class FaceRecognizer:
    ...
    def _load_embeddings_cache(self):
        ...
        self.vip_cache = {}  # {name: embedding_array}
        self.blacklist_cache = {}

        # 加載 VIP 數據
        cursor = self.conn.cursor()
        cursor.execute('SELECT name, face_embedding FROM vip_members WHERE is_active = 1')
        for row in cursor.fetchall():
            try:
                embedding = np.frombuffer(row['face_embedding'], dtype=np.float32)
                self.vip_cache[row['name']] = embedding
            except:
                logger.warning(f"無法加載 VIP {row['name']} 的 embedding")

        # 加載黑名單數據
        cursor.execute('SELECT name, face_embedding FROM blacklist WHERE is_active = 1')
        for row in cursor.fetchall():
            try:
                embedding = np.frombuffer(row['face_embedding'], dtype=np.float32)
                self.blacklist_cache[row['name']] = embedding
            except:
    ...
    def recognize_face(self, face_embedding: np.ndarray) -> Dict:
        ...
        for name, blk_embedding in self.blacklist_cache.items():
        ...
        if best_blacklist_confidence > self.blacklist_threshold:
            ...
            result['details'] = {
                ...
                'distance': self.euclidean_distance(face_embedding, self.blacklist_cache[best_blacklist_match])
            }
            return result
        ...
        for name, vip_embedding in self.vip_cache.items():
        ...
        if best_vip_confidence > self.vip_threshold:
            ...
            result['details'] = {
                ...
                'distance': self.euclidean_distance(face_embedding, self.vip_cache[best_vip_match])
            }
            return result
    ...
    def add_vip(self, name: str, face_embedding: np.ndarray, phone: str = None,
                email: str = None, vip_level: str = 'standard') -> bool:
        """添加 VIP 並更新快取"""
        try:
            cursor = self.conn.cursor()
            cursor.execute('''
                INSERT OR REPLACE INTO vip_members (name, face_embedding, phone, email, vip_level)
                VALUES (?, ?, ?, ?, ?)
            ''', (name, face_embedding.tobytes(), phone, email, vip_level))
            self.conn.commit()

            # 更新快取
            self.vip_cache[name] = face_embedding
            logger.info(f"✓ VIP '{name}' 已添加")
            return True
        except Exception as e:
            logger.error(f"✗ 添加 VIP 失敗: {e}")
            return False

    def add_blacklist(self, name: str, face_embedding: np.ndarray, reason: str = None,
                     risk_level: str = 'medium') -> bool:
        """添加黑名單"""
        try:
            cursor = self.conn.cursor()
            cursor.execute('''
                INSERT OR REPLACE INTO blacklist (name, face_embedding, reason, risk_level)
                VALUES (?, ?, ?, ?)
            ''', (name, face_embedding.tobytes(), reason, risk_level))
            self.conn.commit()

            # 更新快取
            self.blacklist_cache[name] = face_embedding
            logger.info(f"✓ 黑名單 '{name}' 已添加")
            return True
        except Exception as e:
